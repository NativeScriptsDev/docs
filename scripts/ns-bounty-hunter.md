...
- `Config.EvidenceItemName` — capture sonrası kanıt item adı (NSLib RegisterUsableItem ile kayıtlı; kullanılınca info notification döner)
...
- **Mesaj sözlüğü gapleri:** Server bazı error key'leri (`bountyMissing`, `bountyNotActive`, `noIdentifier`, `noActiveGroup`, `bountyNotAccepted`, `notYours`, `noPaperItem`) Config.Messages'ta tanımlı olmadan döndürebilir — bu durumda kullanıcı raw key görür. Çeviri yapılırken eklenmesi tavsiye edilir.
